...
let process = (value, typeCheck, transform) => {
  ...
  if (!(value instanceof Identity)) return Failure.from('invalid argument');
  if (!typeCheck(value.get())) return Failure.from('invalid argument');
...
let mapStringLength = function(value) {
  return process(value, v => Array.isArray(v), v => v.map(str => str.length));
};
