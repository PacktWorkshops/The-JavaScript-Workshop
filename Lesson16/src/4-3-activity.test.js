...
describe('Sentence Functions', () => {
  ...
  test('splitStringIntoList is valid', () => {
    ...
    expect(splitStringIntoList).toBeValidFunctorFunction(
      arg,
      'invalid argument'
    );
  ...
  test('rejectEmptyStrings is valid', () => {
    const arg = ['The', '', 'quick', '', 'brown', '', 'fox'];
    const resp = ['The', 'quick', 'brown', 'fox'];
    expect(rejectEmptyStrings).toBeValidFunctorFunction(
      arg,
      'invalid argument'
    );
  ...
  test('averageStringLength is valid', () => {
    const arg = [3, 5, 5, 3];
    const resp = 4;
    expect(averageStringLength).toBeValidFunctorFunction(
      arg,
      'invalid argument'
    );
